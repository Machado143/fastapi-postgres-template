...
async def test_create_user(db_session: AsyncSession) -> None:
    ...
    data = UserCreate(
        email="svc_test@example.com", password="secret123", full_name="Test User"
    )
...
async def test_list_users(db_session: AsyncSession) -> None:
    ...
    await service.create_user(
        UserCreate(email="list1_svc@example.com", password="secret123")
    )
    await service.create_user(
        UserCreate(email="list2_svc@example.com", password="secret123")
    )
